...
def api_blocked_exit_detection(request):
    ...
    # ── /api/fire-smoke-detection/ ────────────────────────────────────────────────

from .inference_fire import run_fire_smoke_detection

def api_fire_smoke_detection(request):
    """API endpoint pour la détection de feu et fumée (image unique)."""
    if request.method != 'POST':
        return JsonResponse({'status': 'error', 'message': 'Méthode non autorisée'}, status=405)
    ...
    try:
        ...
        if 'image' not in data:
            return JsonResponse(
                {'status': 'error', 'message': 'Image requise pour la détection feu/fumée'}, status=400
            )
        ...
        summary = run_fire_smoke_detection(frame)
        fire_detected = summary['fire_detected']
        smoke_detected = summary['smoke_detected']
        confidence = max((d['confidence'] for d in summary['details']), default=0) / 100
        ...
        if module:
            Detection.objects.create(
                ...
                details={
                    ...
                    'camera': data.get('camera', 'cam_fire'),
                    'detections': summary['details'],
                    'model_version': '1.0.0',
                },
            )
            ...
            if fire_detected or smoke_detected:
                Alert.objects.create(
                    module=module,
                    severity='critical',
                    title='🔥 Feu/Fumée détecté !' if fire_detected else '💨 Fumée détectée !',
                    description=(
                        f'{"Feu et fumée" if fire_detected and smoke_detected else "Feu" if fire_detected else "Fumée"} '
                        f'détecté sur {data.get("camera", "CAM FIRE").upper()} '
                        f'(confiance {confidence:.2%})'
                    ),
                    details={
                        'fire_detected': fire_detected,
        ...
        return JsonResponse({
            ...
            'message': 'Analyse feu/fumée terminée',
        })

    except json.JSONDecodeError:
        return JsonResponse({'status': 'error', 'message': 'Erreur de décodage JSON'}, status=400)
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': f'Erreur détection feu/fumée : {e}'}, status=500)
