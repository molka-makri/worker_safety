...
LIVE_PRELOAD_TARGETS = {
    "fall": "app.fall_detector",
    "fatigue": "app.fatiguedetector",
    "spill": "app.spill_detector",
    "ppe": "app.ppe_detector",
    "manhole": "app.manhole_detector",
    "exit": "app.exit_detector",
    "sign": "app.sign_detector",
    "tracking": "app.worker_tracking_detector",
    "proximity": "app.proximity_detector",
    "posture": "app.posture_detector",
    "panic": "app.panic_detector",
    "fire": "app.fire_smoke_detector",
}
            ...
            "cameras": [
                {
                    ...
                    "chips": [
                        {"label": "CHUTE", "color": "#FF3B2F", "rgb": "255,59,47"}
                    ],
                },
                {
                    "id": 2,
                    ...
                    "chips": [
                        {"label": "FATIGUE", "color": "#FFB800", "rgb": "255,184,0"}
                    ],
                },
            ],
        },
            ...
            "cameras": [
                {
                    ...
                    "chips": [
                        {"label": "SPILL", "color": "#00C2FF", "rgb": "0,194,255"}
                    ],
                },
                {
                    "id": 5,
...
class ReportsView(TemplateView):
    ...
    MODEL_BLUEPRINTS = {
        1: {
            ...
            "pipeline": [
                "Capture video",
                "Detection EPI par bbox",
                "Classification signalisation",
                "Alerte violation",
            ],
            "strengths": [
                "Detection multi-classes",
                "Controle EPI + panneaux",
                "Bounding boxes interpretables",
            ],
        ...
        2: {
            ...
            "pipeline": [
                "Extraction frame",
                "Analyse posture",
                "Score risque ergonomique",
                "Rapport comportement",
            ],
            "strengths": [
                "Vision ergonomique",
                "Interpretation metier",
                "Extensible a MediaPipe",
            ],
        ...
        3: {
            ...
            "pipeline": [
                "Frame video",
                "YOLO closed/open eye",
                "YOLO down/up/bending",
            ...
            "strengths": [
                ...
                "Notifications + tableau dynamique",
            ],
        ...
        5: {
            ...
            "pipeline": [
                "Segmentation fuite",
                "Segmentation manhole",
                "Detection sortie/obstacle",
                "Alerte critique",
            ],
            "strengths": [
                "Masques/polygones",
        ...
        6: {
            ...
            "strengths": [
                "Couverture securite vitale",
                "Extension optique possible",
                "Integration alertes",
            ],
        ...
        7: {
            ...
            "strengths": [
                ...
                "Adaptable aux machines chantier",
            ],
    ...
    def _capture_card(self, det):
        ...
        confidence = max(
            ...
            min(
                100,
                int(round((det.confidence or details.get("confidence") or 0) * 100)),
            ),
        )
        camera = str(
            details.get("camera") or details.get("source") or f"Module {det.module_id}"
        ).upper()
        ...
        if visual == "ppe":
            ...
            badges = [
                details.get("category_name") or details.get("category") or "Panneau",
                details.get("verdict", "DEFECTIVE"),
            ]
        elif visual == "fall":
            ...
            badges = [
                "MANHOLE",
                details.get("manhole_state", "open"),
                f"Risque {details.get('risk_level', 'high')}",
            ]
        elif visual == "exit":
            badges = [
                "EXIT",
                "Obstacle",
                f"{int(details.get('confidence', det.confidence or 0) * 100)}%",
            ]
    ...
    def _build_module_report(self, module, blueprint):
        ...
        coverage = (
            int((len(available_files) / len(model_files)) * 100) if model_files else 0
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        module_reports = [
            self._build_module_report(
                module, self.MODEL_BLUEPRINTS.get(module.id, self.MODEL_BLUEPRINTS[1])
            )
            for module in modules
        ]
        top_module = max(
            module_reports, key=lambda item: item["risk_score"], default=None
        )
        ...
        detection_mix = [
            ...
            for key, value in sorted(
                detection_types.items(), key=lambda item: item[1], reverse=True
            )[:8]
        ]
        ...
        context.update(
            {
                ...
                "kpis": {
                    ...
                    "readiness": (
                        int((model_files_ready / model_files_total) * 100)
                        if model_files_total
                        else 0
                    ),
...
class ModuleDetailView(TemplateView):
    ...
    def _format_detection_rows(self, detections):
        ...
        for det in detections:
            ...
            is_sign_defect = (
                "sign_defect_detected" in details or "is_defective" in details
            )
                                    ...
                                    if is_manhole
                                        ...
                                        if is_ppe_violation
                                            ...
                                            if is_sign_defect
                                                ...
                                                "CAM 8"
                                                if is_proximity or is_worker_tracking
                                                else (
                                                    "CAM 9"
                                                    if is_posture
                                                    else (
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        canonical_slug = normalize_module_slug(slug)
        config = MODULE_PAGE_CONFIG.get(
            canonical_slug, MODULE_PAGE_CONFIG.get(slug, {})
        )
        ...
        context["alerts"] = []
        context["latest_detection_id"] = (
            module.detections.order_by("-id").values_list("id", flat=True).first() or 0
        )
        context["latest_alert_id"] = (
            module.alerts.order_by("-id").values_list("id", flat=True).first() or 0
        )
...
def _draw_bbox(frame, bbox, color, label=None):
    ...
    if label:
        cv2.rectangle(
            frame, (x1, max(0, y1 - 24)), (min(w - 1, x1 + 190), y1), color, -1
        )
        cv2.putText(
            frame,
            str(label)[:24],
...
def _save_detection_capture(
    frame, event_type, camera, confidence, details=None, label=None
):
    ...
    for box_key in ("bbox", "exit_bbox", "obstacle_bbox"):
        _draw_bbox(
            ...
            details.get(box_key),
            color,
            main_label if box_key == "bbox" else box_key.upper(),
        )
...
@csrf_exempt
def hf_media_proxy(request, asset_path):
    safe_path = "/".join(quote(part, safe="._- ") for part in str(asset_path).split("/"))
    upstream_url = f"{HF_MEDIA_DATASET_BASE}/{safe_path}"
    headers = {}
    ...
    try:
        upstream = requests.get(upstream_url, headers=headers, stream=True, timeout=60)
    except requests.RequestException as exc:
        return HttpResponse(f"Media proxy error: {exc}", status=502)

    if upstream.status_code not in (200, 206):
        return HttpResponse(
            f"Upstream media unavailable ({upstream.status_code})",
...
def _send_twilio_sms(body, media_url=None):
    ...
    if media_url and config("TWILIO_SEND_MEDIA_URL", default="False").lower() in (
        "1",
        "true",
        "yes",
        "on",
    ...
    try:
        response = requests.post(
            url, data=payload, auth=(account_sid, auth_token), timeout=8
        )
                                    ...
                                    if is_sign_defect
                                        ...
                                        if is_proximity or is_worker_tracking
                                            ...
                                            if is_posture
                                                ...
                                                "cam10"
                                                if is_panic
                                                else "cam12" if is_fire else "module"
                                            )
                                        )
                                    )
                                )
...
def api_test(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": "Erreur de décodage JSON"}, status=400
            )
    return JsonResponse(
...
def api_module_events(request, slug):
    ...
    if not module:
        return JsonResponse(
            {"status": "error", "message": "Module non trouve"}, status=404
        )
...
def api_clear_alerts(request):
    if request.method != "POST":
        return JsonResponse(
            {"status": "error", "message": "Methode non autorisee"}, status=405
        )
...
def api_preload_models(request):
    if request.method != "POST":
        return JsonResponse(
            {"status": "error", "message": "Methode non autorisee"}, status=405
        )
    ...
    requested_targets = data.get("targets") or list(LIVE_PRELOAD_TARGETS.keys())
    loaded = []
    failed = []

    for target in requested_targets:
        module_path = LIVE_PRELOAD_TARGETS.get(str(target).strip().lower())
        if not module_path:
            failed.append({"target": target, "error": "unknown_target"})
            continue
        try:
    ...
    return JsonResponse(
        {
            ...
            "loaded": loaded,
            "failed": failed,
            "message": "Prechargement des modeles termine",
        }
    )
...
def api_fall_detection(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": f"Erreur de décodage image : {e}"},
                status=400,
            )
        ...
        if module:
            capture_meta = (
                _save_detection_capture(
                    frame,
                    "fall",
                    data.get("camera", "cam1"),
                    confidence,
                    details,
                    "CHUTE",
                )
            ...
            if fall_detected:
                ...
                sms_result = _notify_fall_sms(
                    alert, data.get("camera", "cam1"), confidence, capture_meta
                )
        ...
        return JsonResponse(
            {
                ...
                "capture_url": (
                    capture_meta.get("capture_url") if fall_detected else None
                ),
                "public_capture_url": (
                    _public_media_url(capture_meta.get("capture_url"))
                    if fall_detected
                    else None
                ),
                "sms": sms_result,
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur lors de la détection : {e}"},
            status=500,
        )
...
def api_fall_detection_batch(request):
    ...
    try:
        ...
        if "images" not in data or not isinstance(data["images"], list):
            return JsonResponse(
                {"status": "error", "message": "Liste 'images' requise"}, status=400
            )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur lors du traitement batch : {e}"},
            status=500,
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la détection de fatigue",
                },
                status=400,
            )
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": f"Erreur lors de la détection fatigue : {e}",
            },
            status=500,
        )
...
def api_fatigue_detection_batch(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur batch fatigue : {e}"}, status=500
        )
...
def api_ppe_detection(request):
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {"status": "error", "message": "Image requise pour la detection EPI"},
                status=400,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": f"Erreur de decodage image : {e}"},
                status=400,
            )
        ...
        return JsonResponse(
            {"status": "error", "message": "Erreur de decodage JSON"}, status=400
        )
    except Exception as e:
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection EPI : {e}"}, status=500
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection signalisation",
                },
                status=400,
            )
        ...
        confidence = float(
            result.get("defect_score") or result.get("cat_confidence") or 0
        )
        module = get_module_by_slug("ppe")
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection signalisation : {e}"},
            status=500,
        )
...
def api_proximity_detection(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": f"Décodage: {e}"}, status=400
            )
...
def api_proximity_detection_batch(request):
    ...
    from .proximity_detector import detect_proximity_in_frame
    if request.method != "POST":
        return JsonResponse(
            {"status": "error", "message": "Méthode non autorisée"}, status=405
        )
    ...
    try:
        ...
        for i, image_b64 in enumerate(data["images"]):
            try:
                frame = _decode_image(image_b64)
                proximity_detected, confidence, details = detect_proximity_in_frame(
                    frame
                )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur batch proximité : {e}"}, status=500
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection de deversement",
                },
                status=400,
            )
        ...
        spill_detected, confidence, details = detect_spill_in_frame(
            frame, camera=camera
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection deversement : {e}"},
            status=500,
        )
...
def api_manhole_detection(request):
    ...
    from .manhole_detector import detect_manhole_in_frame
    if request.method != "POST":
        return JsonResponse(
            {"status": "error", "message": "Methode non autorisee"}, status=405
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection de manhole",
                },
                status=400,
            )
        ...
        manhole_detected, confidence, details = detect_manhole_in_frame(
            frame, camera=camera, include_depth=False
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection manhole : {e}"},
            status=500,
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour l estimation de profondeur",
                },
                status=400,
            )
        ...
        if polygon is None and bbox is None:
            detected, confidence, details = detect_manhole_in_frame(
                frame, camera=data.get("camera", "report-depth"), include_depth=False
            )
            if not detected:
                return JsonResponse(
                    {
        ...
        depth_details = estimate_manhole_depth(
            frame, polygon=polygon, bbox=bbox, state=state
        )
        return JsonResponse(
            {
                "status": "success",
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": f"Erreur estimation profondeur manhole : {e}",
            },
            status=500,
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection de sortie",
                },
                status=400,
            )
        ...
        blocked_detected, confidence, details = detect_blocked_exit_in_frame(
            frame, camera=camera
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection sortie de secours : {e}"},
            status=500,
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection incendie",
                },
                status=400,
            )
        ...
        incident_detected, confidence, details = detect_fire_smoke_in_frame(
            frame, camera=camera
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection incendie : {e}"},
            status=500,
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection de posture",
                },
                status=400,
            )
        ...
        unsafe_detected, confidence, details = detect_posture_in_frame(
            frame, camera=camera
        )
        ...
        if module:
            Detection.objects.create(
                ...
                details={
                    ...
                    "camera": camera,
                    "bbox": details.get("bbox"),
                    "keypoints": details.get(
                        "keypoints", details.get("skeleton_keypoints", [])
                    ),
            ...
            if unsafe_detected:
                reasons_str = (
                    ", ".join(details.get("reasons", [])[:2]) or "posture incorrecte"
                )
                Alert.objects.create(
                    module=module,
                    severity="warning",
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection posture : {e}"},
            status=500,
        )
...
def api_worker_tracking_detection(request):
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {"status": "error", "message": "Image requise pour le tracking"},
                status=400,
            )
        ...
        if module:
            Detection.objects.create(
                ...
                details={
                    ...
                    "model_version": "1.0.0",
                    "model_available": details.get("model_available", False),
                    "processing_method": details.get(
                        "processing_method", "yolo_deepsort_tracking_line_crossing"
                    ),
                },
            )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur tracking workers : {e}"}, status=500
        )
    ...
    try:
        ...
        if "image" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Image requise pour la detection de panique",
                },
                status=400,
            )
        ...
        panic_detected, confidence, details = detect_panic_in_frame(
            frame, camera=camera
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erreur detection panique : {e}"},
            status=500,
        )
...
def api_chat(request):
    ...
    try:
        ...
        if not user_message:
            return JsonResponse(
                {"status": "error", "message": "Message vide"}, status=400
            )
        ...
        if not api_key:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Clé API OpenRouter non configurée dans le fichier .env",
                },
                status=500,
            )
        ...
        return JsonResponse(
            {"status": "error", "message": "Erreur de connexion à l'IA"}, status=500
        )
    except Exception as e:
        return JsonResponse(
