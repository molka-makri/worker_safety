...
let cam9DetectionInterval      = null;
let cam9VideoAnalysisActive    = false;

let cam10DetectionInterval     = null;
let cam10VideoAnalysisActive   = false;
...
const POSTURE_ANALYSIS_INTERVAL_MS  = 1000;
const POSTURE_ALERT_COOLDOWN_MS     = 5000;

// Panic State Variables (CAM 10)
let panicRequestInFlight            = false;
let panicLastAlertAt                = 0;
const PANIC_ANALYSIS_INTERVAL_MS    = 600;
...
function drawPostureOverlay(canvasId, details, video, isUnsafe) {
  ...
  const kpts        = Array.isArray(details.keypoints) ? details.keypoints : [];  // [[x,y], ...]
  ...
  if (reasons.length > 0 || !isUnsafe) {
    ...
    ctx.font      = 'bold 11px monospace';
    ctx.fillStyle = color;
    const tag = isUnsafe ? `⚠ UNSAFE  ${conf}%` : `✓ SAFE  ${conf}%`;
    ctx.fillText(tag, 12, 22);

    ctx.font      = '10px monospace';
    ctx.fillStyle = '#ddd';
    reasons.slice(0, 4).forEach((r, i) => {
      ctx.fillText('• ' + r, 12, 36 + i * 14);
    });
  }
}
...
function drawPanicOverlay(canvasId, details, video, isPanic) {
  ...
  const color   = warming ? 'rgba(0,194,255,0.80)'
                : isPanic ? 'rgba(255,59,47,0.95)' : 'rgba(74,227,181,0.95)';
  const conf    = Math.round((details.confidence || 0) * 100);
  ...
  ctx.font      = 'bold 13px monospace';
  ctx.fillStyle = color;
  ctx.fillText(warming ? 'WARMING UP...' : (isPanic ? '🚨 PANIC' : '✓ NORMAL'), 14, 28);
  ...
  if (!warming) {
    ...
    ctx.fillStyle = isPanic ? 'rgba(255,59,47,0.90)' : 'rgba(74,227,181,0.90)';
...
function startCameras() {
  ...
  // cam8 uses upload button — not auto-started

  // cam9 and cam10: videos already have src in HTML, so play directly
  const cam9 = document.getElementById('cam9-video');
  if (cam9) {
    cam9.currentTime = 0;
    cam9.play().catch(() => {});
    startCam9Detection(cam9);
  }

  const cam10 = document.getElementById('cam10-video');
  if (cam10) {
    cam10.currentTime = 0;
    cam10.play().catch(() => {});
    startCam10Detection(cam10);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const cam9 = document.getElementById('cam9-video');
  if (cam9) {
    cam9.addEventListener('loadeddata', () => {
      if (cam9.paused && !cam9VideoAnalysisActive) updateCam9Status('Caméra prête');
    });
    cam9.addEventListener('play', () => { if (!cam9VideoAnalysisActive) startCam9Detection(cam9); });
    cam9.addEventListener('pause', () => updateCam9Status('Vidéo en pause'));
  }

  const cam10 = document.getElementById('cam10-video');
  if (cam10) {
    cam10.addEventListener('loadeddata', () => {
      if (cam10.paused && !cam10VideoAnalysisActive) updateCam10Status('Caméra prête');
    });
    cam10.addEventListener('play', () => { if (!cam10VideoAnalysisActive) startCam10Detection(cam10); });
    cam10.addEventListener('pause', () => updateCam10Status('Vidéo en pause'));
  }
});